...
$serverPod = kubectl get pods -l app=fl-server -o jsonpath='{.items[0].metadata.name}'
...
kubectl exec $serverPod -- python /app/shared/plot_results.py
...
kubectl cp "${serverPod}:/results/history" ./az_results/history
kubectl cp "${serverPod}:/results/plots" ./az_results/plots
